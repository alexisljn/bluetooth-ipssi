...
@app.route('/devices/<mac_address>/delete-scan', methods=['POST'])
def delete_scan(mac_address):
    post = request.form
    if 'scan_id' not in post:
        return redirect('/devices/' + mac_address)

    db = mysql_communicator.connector()
    db.delete_scan(post['scan_id'])
    return redirect('/devices/' + mac_address)
